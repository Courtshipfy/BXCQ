...
- `一键打包Windows版.bat`：生成 64 位 Windows 发布包。如果电脑没有 Godot 4.7 .NET、.NET 8 SDK 或 Godot 导出模板，会自动下载到 `.tools/`，不需要管理员权限。每次打包会在 `build/windows/` 下产生一个带时间戳的目录和 zip 包，不覆盖之前的产物。
